...
func readARP(handle *pcap.Handle, iface *net.Interface, stop chan struct{}) {
	...
	for {
		...
		case packet = <-in:
			...
			tempIp := fmt.Sprint(net.IP(arp.SourceProtAddress))
			tempMac := fmt.Sprint(net.HardwareAddr(arp.SourceHwAddress))
			var tempWhitelisted bool = false
			whitlistResultIp := gjson.Get(string(whitelistData), "#(ip=="+tempIp+")")
			if whitlistResultIp.Exists() {
				//IP is whitlisted
				//Check mac now:
				whitlistResultMac := gjson.Get(string(whitelistData), "#(mac=="+tempMac+")")
				if whitlistResultMac.Exists() {
					// Both checks are good
					// fmt.Println("Whitelist OK for:", tempIp)
					tempWhitelisted = true
				} else {
					fmt.Println("Whitelist mac check failed on", tempIp, tempMac)
				}
			} else {
				fmt.Println("Not whitelisted", tempIp, tempMac)
				// vendor = "Unknown"
			}

			mutex.Lock()
			devices[fmt.Sprint(net.IP(arp.SourceProtAddress))] = device{ip: fmt.Sprint(net.IP(arp.SourceProtAddress)), mac: fmt.Sprint(net.HardwareAddr(arp.SourceHwAddress)), vendor: vendor, lastseen: time.Now().Unix(), whitelisted: tempWhitelisted}
			mutex.Unlock()
...
func handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	var metricName string = "networkguard"
	w.Write([]byte(fmt.Sprintln("# HELP", metricName, "Online network devices.")))
	w.Write([]byte(fmt.Sprintln("# TYPE", metricName, "gauge")))
	for _, k := range keys {
		whitelistedAsInteger := 0
		if devices[k].whitelisted {
			whitelistedAsInteger = 1
		}
		w.Write([]byte(fmt.Sprintf("%v{ip=\"%v\", mac=\"%v\", vendor=\"%v\"} %v\n", metricName, k, devices[k].mac, devices[k].vendor, whitelistedAsInteger))) //devices[k]
	}
}
